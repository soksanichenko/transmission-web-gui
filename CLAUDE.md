...
A custom web UI for the Transmission BitTorrent daemon. Dense, information-rich interface in the spirit of µTorrent 2.x. React 19 + TypeScript + Vite frontend, FastAPI config backend, deployed via Ansible to home-server (`zelgray.cherkasy.ua`), served at `https://portal.zelgray.cherkasy.ua/transmission-ui/`.
